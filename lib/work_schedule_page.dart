...
void extractAshSchedule(String extractedText) {
  // Normalize the text by replacing multiple spaces with a single space and trimming
  String normalizedText = extractedText.replaceAll(RegExp(r'\s+'), ' ').trim();

  // Print the normalized text for debugging
  print("Normalized Text:\n$normalizedText");

  // Regex pattern to match ASH's schedule
  ...
  if (match != null) {
    ...
    ashSchedule.updateAll((day, time) => time == null || time.isEmpty ? "OFF" : time);
